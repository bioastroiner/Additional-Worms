...
public final class ParticleHelper {
    ...
    public static void spawnParticles(IWorld worldIn, BlockPos posIn, int data, ParticleType particleType) {
        ...
        if (!blockstate.isAir(worldIn, posIn)) {
            for (int i = 0; i < data; ++i) {
                ...
                worldIn.addParticle((IParticleData) particleType,
                        (float) posIn.getX() + random.nextFloat(),
                        (double) posIn.getY() + (double) random.nextFloat() * blockstate.getShape(worldIn, posIn).getEnd(Direction.Axis.Y),
                        (float) posIn.getZ() + random.nextFloat(),
                        d0, d1, d2);
            }
    ...
    public static void spawnBonemealParticles(IWorld worldIn, BlockPos posIn, int data) {
        spawnParticles(worldIn, posIn, data,
