...
public class WormItemRed extends WormItemBase {
    ...
    protected boolean placeWorm(PlayerEntity player, BlockPos pos, World world, ItemStack stack) {
        ...
        List<WormEntityRed> worms = world.getEntitiesWithinAABB(WormEntityRed.class, new AxisAlignedBB(pos.getX() - 1, pos.getY(), pos.getZ() - 1, pos.getX() + 2, pos.getY() + 1, pos.getZ() + 2));
        if (worms == null || worms.isEmpty()) {
            if (!world.isRemote) {
                WormEntityRed worm = new WormEntityRed(ModEntityTypes.WORM_ENTITY_RED.get(),world);
